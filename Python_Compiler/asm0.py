...
-- EA is nopop
  ...
  type rom_3 is array (65529 to 65535) of std_logic_vector(7 downto 0); --65535
  constant pla_end : rom_3 := (
    --FFFA   FFFB   NMI
      x"00", x"00",
    --FFFC   FFFD   Reset
      x"00", x"00",
  ...
  type rom_2 is array (65530 to 513) of std_logic_vector(7 downto 0); --65535
  constant others : rom_2 := (
    '''

    z = 0
    for i in range(65530 - 513):
        s += 'x"00",'
        z = z + 1
        if z >= 10:
            s += '\n'
...
def handle_memory_location(token):
    ...
    if location == 'NPAGE':
        ...
        if address >= current_address:
            ...
            error('New address (' + str(address) + ') cant be smaller than current address (' + str(
                current_address) + ')')
    ...
    for byte in memory_fp:
        ...
        if isinstance(byte.value, int):
            ...
            if is_byte:
                ...
                if delta < 0:
                    if delta < -128:
                        error('Relative jump too big from address {:04X} to {:04X} ({:5d})'.format(current_address, val,
                                                                                                   delta))
                    else:
